...
0. generate the demo keys and certificates by running ```go run ./example/utils/gencerts``` from the
   repo root. They are throwaway material and are not committed, so this step is required before the
   first run.
